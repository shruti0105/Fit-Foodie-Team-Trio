...
def dashboard():
    if 'loggedin' in session:
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM nutrients WHERE u_id = %s order by time desc', (session['id'],))
...
def getDailyReport():
    ...
    if 'loggedin' in session:
       ...
       return render_template('charts.html', quantity=json.dumps(quantity), durations=json.dumps(durations), nutrient=nutrient, period=period,isPost=isPost,login=True)
    # else:
    #   if len(quantity)>0:
    #     isPost=True
...
@app.route('/fitFoodie/recommend', methods =['GET', 'POST'])
def recommend():
    # Remove session data, this will log the user out
   ...
   if request.method == "POST":
        ...
        # print(food)
        client = Edamam(nutrition_appid='1d4b55d5',
           nutrition_appkey='f1e1311e07fc57a63cfc5165c25d4cb2',
           recipes_appid='6bcf14f0',
           recipes_appkey='87e35c3c86b17ea771df34d2c5c835aa',
           food_appid='7179266d',
           food_appkey='cb1028fa751edbeb9f0ecc07b4175295')
        recipe=client.search_recipe(food)
        print(recipe)
        for i in recipe:
            print(i)
        url=recipe.get('url')
        print(url)
        context = client.search_recipe(food)
        return render_template('recommend.html', context= context)
